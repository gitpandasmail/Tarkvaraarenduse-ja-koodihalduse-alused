...
#install.packages("tidyverse")  
library(tidyverse)             
Islander_data <- read_csv("../../data/Islander_data.csv")
...
# Kontrollime, kas uus veerg tekkis
head(Islander_data[, c("age", "age_group")], 10)
...
ggplot(Islander_data, aes(x = Drug, y = Diff)) +
  geom_violin(fill = "lightblue", color = "black", alpha = 0.6) +
  geom_point(
    position = position_jitter(width = 0.2, height = 0),
    alpha = 0.2,   # teeb punktid heledaks/läbipaistvaks
    color = "darkblue"
  ) +
  labs(
    title = "Ravimi mõju mälu taastamisele",
    x = "Ravim",
    y = "Mälu skooride erinevus (Diff)"
  ) +
  theme_minimal()
...
ggplot(Islander_data, aes(x = Drug, y = Diff)) +
  geom_jitter(aes(color = Drug), width = 0.2, alpha = 0.3, size = 2) +
  theme_minimal()
...
ggplot(Islander_data, aes(x = Drug, y = Diff, fill = age_group)) +
  geom_violin(alpha = 0.7, color = NA) +
  geom_jitter(aes(color = age_group), width = 0.15, alpha = 0.3, size = 1.5) +
  scale_fill_brewer(palette = "Set2") +
  scale_color_brewer(palette = "Set2") +
  theme_minimal() +
  theme(legend.position = "none")  # eemaldame legendi


ggplot(Islander_data, aes(x = Drug, y = Diff, fill = age_group)) +
  geom_violin(alpha = 0.7, color = NA) +
  geom_jitter(aes(color = age_group), width = 0.15, alpha = 0.25, size = 1.5) +
  scale_fill_brewer(palette = "Pastel1") +
  scale_color_brewer(palette = "Pastel1") +
  labs(
    title = "Ravimite mõju mälu taastamisele Islander andmestikus",
    x = "Ravim",
    y = "Mälu skoori muutus (Diff)"
  ) +
  theme_minimal() +
  theme(
